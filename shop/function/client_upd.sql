...
BEGIN
    ...
    SELECT coalesce(s.client_id, nextval('shop.shop_sq')) AS client_id,
           s.name,
           s.phone,
           s.is_deleted
    ...
    IF exists(SELECT 1 FROM shop.client c WHERE c.phone = _phone AND c.client_id = _client_id)
    THEN
        RETURN public.errmessage(_errcode := 'shop.client_ins.same_phone',
                                 _msg := 'Такой номер телефона уже принадлежит этому пользователю!',
                                 _detail := concat('phone = ', _phone, ' client_id = ', _client_id));
    END IF;
